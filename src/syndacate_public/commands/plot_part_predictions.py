...
import juml
from syndacate_public.datasets.syndacate import shapes

class PlotPartPredictions(juml.base.Command):
    @classmethod
    def run(
        cls,
        args:       cli.ParsedArgs,
        ...
        plot_type.plot(mp, "part_predictions", model_dir)
    ...
    def plot_set(
        ...
        **kwargs,
        ...
        t_dim = shapes.Line.get_pose_names().index("thickness")
        b_dim = shapes.Line.get_pose_names().index("brightness")
        ...
        return [
            line
            for p in poses.tolist()
            for line in shapes.Line.from_pose(np.array(p)).plot(**kwargs)
        ]
